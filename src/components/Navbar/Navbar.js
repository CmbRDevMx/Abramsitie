...
function Navbar() {
 ...
 <div className="container mx-auto px-4 py-3 grid grid-cols-8 gap-4 items-center">
  ...
  <div className="col-span-3 md:col-span-5  flex justify-center md:justify-start">
    <input type="checkbox" id="menu-toggle" className="hidden" checked={toggleMenu} onChange={handleToggle} />
    <div className="md:flex items-center text-gray-700 text-[24px] justify-start space-x-4">
      ...
      <label htmlFor="menu-toggle" className="menu-icon cursor-pointer md:hidden">
        <span className={`navicon bg-gray-700 block w-6 h-0.5 ${toggleMenu ? 'rotate-45' : ''}`}></span>
        <span className={`navicon bg-gray-700 block w-6 h-0.5 mt-1 ${toggleMenu ? 'opacity-0' : ''}`}></span>
        <span className={`navicon bg-gray-700 block w-6 h-0.5 mt-1 ${toggleMenu ? 'rotate--45' : ''}`}></span>
      </label>
    </div>
  </div>
  <div className='col-span-3 md:col-span-2 flex justify-end'>
  <button className="btn py-[10px] px-[30px] rounded-md col-span-2" onClick={handleToggle}>Book a Call</button>
  </div>
</div>
